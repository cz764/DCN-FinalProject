package dcn.finalProject;

/* ------------------
   Server
   usage: java Server [RTSP listening port]
   Description: Used to listen to the request from the client and receive the RTSP messages.
   ---------------------- */
...
//RTP variables:
  ...
  public static void main(String argv[]) throws Exception {
    ...
    while(true) {
    	//parse the request
    	request_type = theServer.parse_RTSP_request(); //get the parsed rtsp request
    	...
    	if ((request_type == PLAY) && (state == READY)) {
    	...
    	else if ((request_type == DESCRIBE) && ((state == READY) || (state == PLAYING))) {
    		theServer.send_RTSP_response();
    		//stop timer
    		theServer.timer.stop();
    		//update state
    		state = READY;
    		System.out.println("New RTSP state: READY");	
    		System.out.println("Content-base: rtsp://JAVA/workspace/Testing/" + VideoFileName);
    		System.out.println("Content-Type: application/sdp" );
    		System.out.println("Content-Length: 430");
    		System.out.print("m=video 0 RTP/AVP 96\na=control:streamid=0\na=range:npt=0-7.741000\na=length:npt=7.741000\na=rtpmap:96 MP4V-ES/5544\n"
  ...
  int parse_RTSP_request() {
	  ...
	  try {
		  ...
		  // If the request is SETUP, we need to get the video file name first
		  if (request_type == SETUP) {
		  ...
		  if (request_type == SETUP) {
			  ...
			  for (int i = 1; i <=4; i++){
				  //System.out.println("nextToken: " + tokens.nextToken());
				  tokens.nextToken(); //skip unused stuff
			  }
			  int port = Integer.parseInt(tokens.nextToken());
			  //System.out.println("destPort: " + Integer.parseInt(tokens.nextToken()));
			  //RTP_dest_port = port;
		  }
